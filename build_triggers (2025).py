...
MASTER = "/mnt/user-data/uploads/NFL_Schedule__NEW___12_.xlsx"
TRIG   = "/mnt/user-data/uploads/NFL_Schedule_Triggers__2026_.xlsx"
...
# Trigger definitions — labels match the legend (Rest +3 / TZ +3 / Rest -3 / TZ -3)
TRIGGERS = [
    ...
    {'code':'P5a','label':'Cold v Hot',    'polarity':'P', 'desc':'Focal home in Cold venue; opp Warm/Dome; Date >= Nov 15'},
    {'code':'P5b','label':'Early Hot Host','polarity':'P', 'desc':'Focal = MIA/JAX/TB home vs Cold opp; Week 1-2'},
    ...
    {'code':'N7a','label':'Hot v Cold',    'polarity':'N', 'desc':'Focal away (Warm/Dome) at Cold venue; Date >= Nov 15'},
    {'code':'N7b','label':'Early Hot Road','polarity':'N', 'desc':'Focal away (Cold) at MIA/JAX/TB; Week 1-2'},
    {'code':'N7c','label':'@ DEN',         'polarity':'N', 'desc':'Focal away at Denver'},
]
...
def build(season, out_path):
    ...
    for r in range(2, ws_t.max_row + 1):
        ...
        fired = [code for code, col in trig_col.items()
                 if ws_t.cell(row=r, column=col).value == 1]
        if not fired:
            continue

        f1_pos = int(ws_t.cell(row=r, column=th['F1|Pos']).value or 0)
        f1_neg = int(ws_t.cell(row=r, column=th['F1|Neg']).value or 0)
        game = {'week': week, 'date': date_iso, 'opp': opp, 'han': han,
                'triggers': fired, 'net': f1_pos - f1_neg}
